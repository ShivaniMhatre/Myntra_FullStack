...
const Add_Product = () => {
    ...
    const [productData, setProductData] = useState({ name: '', price: '', d_price: '', image: '', category: 'Other' })
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (productData.name && productData.category && productData.price && productData.d_price && productData.image) {
            const token = JSON.parse(localStorage.getItem('Token'))
            try {
                const response = await axios.post('http://localhost:4001/addproduct', { productData, token })
                if (response.data.success) {
                    console.log(response)
                    setProductData({ name: '', price: '', d_price: '', image: '', category: 'Other' })
                    route('/yourProduct')
    ...
    //     if (productData.name && productData.price && productData.d_price && productData.img) {
    //         const product_array = JSON.parse(localStorage.getItem('Myntra_Product')) || [];
    //         const randomId = uuid4();
    //         productData["id"] = randomId;
    //         product_array.push(productData);
    //         localStorage.setItem('Myntra_Product', JSON.stringify(product_array));
